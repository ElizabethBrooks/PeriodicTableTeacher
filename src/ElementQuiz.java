...
public class ElementQuiz {
	...
	public void setRandomQuizQuestion(){
		...
		atomicNumQuestion = randomAtomicNum.nextInt(118);		
	}
	
	//Quiz the user on elements based on their atomic number
	public  getElementQuiz(){
		...
		while (invalidEntry)
		{
			...
			{			
				...
				if(quizYesOrNo.equals("YES")){
					userAnswer = JOptionPane.showInputDialog("What element has an atomic number of " + quiz.getRandomQuizQuestion());
			...
			//Check the users answer to determine if they answered the questions correctly.
	}
